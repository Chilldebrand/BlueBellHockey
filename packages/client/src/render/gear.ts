...
export function buildStick(): THREE.Group {
...
export function attachStick(root: THREE.Object3D): void {
...
export function buildGlove(team: number): THREE.Group {
  const g = new THREE.Group();
  const mat = new THREE.MeshStandardMaterial({ color: TEAM_GEAR[team] ?? '#888', roughness: 0.5 });
...
export function attachGloves(root: THREE.Object3D, team: number): void {
  for (const name of ['hand.l', 'hand.r']) {
    const bone = findBone(root, [name]);
    if (bone) bone.add(buildGlove(team));
  }
}
...
/** Attach the full kit. */
export function attachGear(root: THREE.Object3D, team: number): void {
  attachStick(root);
  attachGloves(root, team);
